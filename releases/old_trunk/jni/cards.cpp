...
void Game::scaleCards()
{
  for (auto& card: cards)
  {
    ...
    if (card.shape.getSize().y != (cardSize.y * card.shape.getSize().x) / cardSize.x)
      card.shape.setSize( sf::Vector2f(card.shape.getSize().x, (cardSize.y * card.shape.getSize().x) / cardSize.x) );
  }
}
...
sf::Vector2f Game::absoluteCardPosition(unsigned int x, unsigned int y) // returns the position of a card at board position (x,y)
{
  ...
  // cards are offset by 1/2 the spacing between cards.
